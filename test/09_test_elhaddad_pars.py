...
config_matplotlib(font_size=14, font_family="serif", use_latex=True, interactive=False)
cf = get_config_file(parse_arguments("./09_test_elhaddad.yaml"))

#%% Istantiate El Haddad curve
eh = ElHaddadCurve(dk_th=cf["curve"]["dk_th"],
                   ds_w=cf["curve"]["ds_w"],
                   y=cf["curve"]["y"],
                   name=cf["id"],
                   metrics=getattr(np, cf["curve"]["metrics"]))
...
p = p = PreProViewer([1,1000], [50, 800], 1000, "log", 0.65)
#p.view(train_data=sd)
#p.view(test_data=sd)6
